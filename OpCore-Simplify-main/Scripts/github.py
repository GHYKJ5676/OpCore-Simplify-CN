...
class Github:
    ...
    def get_commits(self, owner, repo, branch="main", start_commit=None, after=-1):
        ...
        if after < 0:
            url = PROXY_PREFIX + "https://github.com/{}/{}/commits/{}".format(owner, repo, branch)
        else:
            url = PROXY_PREFIX + "https://github.com/{}/{}/commits/{}?after={}+{}".format(owner, repo, branch, start_commit, after)
        response = self.fetcher.fetch_and_parse_content(url)
        if not response:
            raise ValueError("从 GitHub 获取提交信息失败。")
        payload = self.extract_payload(response)
        if not "commitGroups" in payload:
            raise ValueError("无法在仓库 {} 的分支 {} 上找到提交信息。".format(repo, branch))
        return payload

    def get_latest_release(self, owner, repo):
        url = PROXY_PREFIX + "https://github.com/{}/{}/releases".format(owner, repo)
        response = self.fetcher.fetch_and_parse_content(url)
        if not response:
            raise ValueError("从 GitHub 获取发布信息失败。")
        tag_name = self._extract_tag_name(response)
        body = self._extract_body_content(response)
        release_tag_url = PROXY_PREFIX + "https://github.com/{}/{}/releases/expanded_assets/{}".format(owner, repo, tag_name)
        response = self.fetcher.fetch_and_parse_content(release_tag_url)
        if not response:
            raise ValueError("从 GitHub 获取展开资产信息失败。")
            ...
            if "<li" in line:
                ...
                if download_link and asset_id:
                    assets.append({
                        ...
                        "id": int(asset_id), 
                        "url": PROXY_PREFIX + "https://github.com" + download_link,
                        "sha256": sha256
                    })
                in_li_block = False
            if in_li_block:  
