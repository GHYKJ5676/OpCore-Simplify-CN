...
PROXY_PREFIX = "https://ghfile.geekertao.top/"
...
class gatheringFiles:
    def __init__(self):
        ...
        self.dortania_builds_url = PROXY_PREFIX + "https://raw.githubusercontent.com/dortania/build-repo/builds/latest.json"
        self.ocbinarydata_url = PROXY_PREFIX + "https://github.com/acidanthera/OcBinaryData/archive/refs/heads/master.zip"
        self.amd_vanilla_patches_url = PROXY_PREFIX + "https://raw.githubusercontent.com/laobamac/AMD_Vanilla/refs/heads/master/patches.plist"
        self.aquantia_macos_patches_url = PROXY_PREFIX + "https://raw.githubusercontent.com/CaseySJ/Aquantia-macOS-Patches/refs/heads/main/CaseySJ-Aquantia-Patch-Sets-1-and-2.plist"
        self.hyper_threading_patches_url = PROXY_PREFIX + "https://github.com/b00t0x/CpuTopologyRebuild/raw/refs/heads/master/patches_ht.plist"
    ...
    def gather_bootloader_kexts(self, kexts, macos_version):
        self.utils.head("正在收集文件")
        print("")
        print("请等待下载 OpenCorePkg、kext 和 macserial...")
        ...
        for product in kexts + [{"Name": "OpenCorePkg"}]:
            ...
            if product_info is None:
                print("\n")
                print("找不到 {} 的下载 URL。".format(product_name))
                continue
            ...
            if history_item is not None:
                ...
                if is_latest_id and folder_is_valid:
                    print(f"\n{product_name} 已是最新版本，无需下载。")
                    continue
            print("")
            print("正在更新" if history_item is not None else "请等待下载", end=" ")
            print("{}...".format(product_name))
            if product_download_url:
                print("来源：{}".format(product_download_url))
                print("")
            else:
                print("")
                print("找不到 {} 的下载 URL。".format(product_name))
            ...
            if not self.fetcher.download_and_save_file(product_download_url, zip_path, sha256_hash):
                ...
                if history_item is not None and folder_is_valid:
                    print("使用之前下载的 {} 版本。".format(product_name))
                    continue
                else:
                    raise Exception("当前无法下载 {}，请稍后重试。".format(product_name))
            ...
            if "OpenCore" in product_name:
                ...
                print("来源：{}".format(self.ocbinarydata_url))
                print("")
                ...
                if not os.path.exists(oc_binary_data_zip_path):
                    print("")
                    print("当前无法下载 OcBinaryData，请稍后重试。\n")
                    self.utils.request_input()
                    shutil.rmtree(self.temporary_dir, ignore_errors=True)
                    return False
    ...
    def get_kernel_patches(self, patches_name, patches_url):
        try:
            ...
            print("")
            print("当前无法下载 {}。".format(patches_name))
            print("来源：" + patches_url)
            print("")
            print("请稍后重试或手动应用补丁。")
            print("")
            self.utils.request_input()
    ...
    def gather_hardware_sniffer(self):
        if os_name != "Windows":
            return
        self.utils.head("正在获取 Hardware Sniffer")
        ...
        if not all([product_id, product_download_url, sha256_hash]):
            print("")
            print("找不到 {} 的发布信息。".format(PRODUCT_NAME))
            print("请稍后重试。")
            print("")
            self.utils.request_input()
            raise Exception("找不到 {} 的发布信息。".format(PRODUCT_NAME))
        local_download_history = self._load_download_history()
        history_item = local_download_history.get(PRODUCT_NAME)
        if history_item is not None:
            is_latest_id = (product_id == history_item.get("id"))
            ...
            if is_latest_id and file_is_valid:
                print("")
                print("{} 已是最新版本。".format(PRODUCT_NAME))
        ...
        if not self.fetcher.download_and_save_file(product_download_url, destination_path, sha256_hash):
            ...
            print("请前往 {} 手动下载 {}。".format(manual_download_url, PRODUCT_NAME))
            print("")
            self.utils.request_input()
