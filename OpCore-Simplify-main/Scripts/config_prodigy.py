...
class ConfigProdigy:
    ...
    def select_audio_codec_layout(self, hardware_report, config=None, controller_required=False):
        ...
        while True:
            ...
            contents.append("- 请尝试不同布局以找到最适合您系统的。")
            contents.append("")
            content = "\n".join(contents)
            self.utils.adjust_window_size(content)
            self.utils.head("选择音频布局 ID", resize=False)
            print(content)
            selected_layout_id = self.utils.request_input(f"请输入您要使用的音频布局 ID（默认：{default_layout.id}）：") or default_layout.id
            try:
    ...
    def load_kernel_patch(self, motherboard_chipset, cpu_manufacturer, cpu_codename, cpu_cores, gpu_manufacturer, networks, macos_version, kexts, provide_current_cpu_info):
        ...
        if "AMD" in cpu_manufacturer:
            kernel_patch.extend(self.g.get_kernel_patches("AMD Vanilla 补丁", self.g.amd_vanilla_patches_url))
        if any(network_props.get("Device ID") in pci_data.AquantiaAqtionIDs for network_props in networks.values()):
            kernel_patch.extend(self.g.get_kernel_patches("Aquantia macOS 补丁", self.g.aquantia_macos_patches_url))
        if provide_current_cpu_info and kexts[kext_data.kext_index_by_name.get("CpuTopologyRebuild")].checked:
            kernel_patch.extend(self.g.get_kernel_patches("超线程补丁", self.g.hyper_threading_patches_url))
        elif kexts[kext_data.kext_index_by_name.get("ForgedInvariant")].checked:
            if not "AMD" in cpu_manufacturer:
                kernel_patch.extend(self.g.get_kernel_patches("AMD Vanilla 补丁", self.g.amd_vanilla_patches_url)[-6:-4])
        elif kexts[kext_data.kext_index_by_name.get("CatalinaBCM5701Ethernet")].checked:
            kernel_patch.append({
                "Arch": "Any",
                "Base": "",
                "Comment": "Broadcom BCM577XX 补丁",
                "Count": 1,
                "Enabled": True,
