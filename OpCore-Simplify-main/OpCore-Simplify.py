...
class OCPE:
    ...
    def select_hardware_report(self):
        ...
        while True:
            ...
            if os.name == "nt":
                ...
                print("- Hardware Sniffer 在 Windows PE 中不会收集与 GPU 的 Resizable BAR 选项（默认禁用）和显示器连接相关的信息。")
                print("")
                print("E. 导出硬件报告（推荐）")
                print("")
            print("Q. 退出")
            ...
            user_input = self.u.request_input("请将硬件报告（.JSON）拖放到此处{}：".format("或输入 \"E\" 导出" if os.name == "nt" else ""))
            ...
            if not is_valid or errors:
                print("")
                print("\033[32m建议：\033[0m请重新导出硬件报告并重试。")
                print("")
                self.u.request_input("按 Enter 返回...")
            else:
                return path, data
    ...
    def show_oclp_warning(self):
        while True:
            ...
            print("")
            print("1. OpenCore Legacy Patcher 是在较新 macOS 版本中启用被放弃的 GPU 和 Broadcom WiFi 支持的唯一解决方案，同时也是为 macOS Tahoe 26 恢复 AppleHDA 的方法。")
            print("")
            print("2. OpenCore Legacy Patcher 会禁用 SIP 和 AMFI 等 macOS 安全功能，可能导致需要完整安装程序进行更新、应用程序崩溃和系统不稳定等问题。")
            print("")
            print("3. OpenCore Legacy Patcher 在 Hackintosh 社区中不受官方支持。")
            print("")
            print("\033[1;91m重要：\033[0m")
            print("请在继续之前仔细考虑这些风险。")
            print("")
            print("\033[1;96m对 macOS Tahoe 26 的支持：\033[0m")
            print("要修补 macOS Tahoe 26，您必须从我的 GitHub 仓库 \033[4mlzhoang2801/OpenCore-Legacy-Patcher\033[0m 下载 OpenCore-Patcher 3.0.0 或更新版本。")
            print("较旧或官方的 Dortania 版本不支持 Tahoe 26。")
            print("")
            option = self.u.request_input("您要继续使用 OpenCore Legacy Patcher 吗？(yes/No): ").strip().lower()
            if option == "yes":
                return True
            elif option == "no":
                return False
    ...
    def before_using_efi(self, org_hardware_report, hardware_report):
        ...
        print("    - 将生成的 UTBMap.kext 添加到 {} 文件夹中。".format("EFI\\OC\\Kexts" if os.name == "nt" else "EFI/OC/Kexts"))
        print("    - 删除 {} 文件夹中的 UTBDefault.kext。".format("EFI\\OC\\Kexts" if os.name == "nt" else "EFI/OC/Kexts"))
    ...
    def main(self):
        ...
        while True:
            ...
            print("3. 自定义 ACPI 补丁")
            print("4. 自定义 Kexts")
            print("5. 自定义 SMBIOS 机型")
            print("6. 构建 OpenCore EFI")
            print("")
            print("Q. 退出")
            print("")

            option = self.u.request_input("请选择选项：")
            if option.lower() == "q":
                self.u.exit_program()
            ...
            if not hardware_report_path:
                self.u.head()
                print("\n\n")
                print("\033[1;93m请先选择硬件报告。\033[0m")
                print("\n\n")
                self.u.request_input("按 Enter 返回...")
                continue
            ...
            if option == "2":
                ...
                try:
                    ...
                    print("")
                    self.u.request_input("按 Enter 继续...")
                    continue
                ...
                self.u.head("结果")
                print("")
                print("您的 {} 的 OpenCore EFI 已构建于：".format(customized_hardware.get("Motherboard").get("Name")))
                print("\t{}".format(self.result_dir))
                print("")
                self.u.request_input("按 Enter 返回主菜单...")
