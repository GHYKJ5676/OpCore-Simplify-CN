...
class SMBIOS:
    ...
    def check_macserial(self, retry_count=0):
        ...
        if os_name == "Windows":
            ...
            raise Exception("未知操作系统，无法找到 macserial")
        ...
        if retry_count >= max_retries:
            raise Exception("在 {} 次尝试后仍未找到 macserial".format(max_retries))
    ...
    def customize_smbios_model(self, hardware_report, selected_smbios_model, macos_version):
        ...
        while True:
            ...
            contents.append("")
            if show_all_models:
                contents.append("所有可用 SMBIOS 列表：")
            else:
                contents.append("兼容的 SMBIOS 列表：")
            ...
            contents.append("- 灰色行表示该 Mac 型号不被 {} 官方支持。".format(os_data.get_macos_name_by_darwin(macos_version)))
            contents.append("")
            ...
            if selected_smbios_model != default_smbios_model:
                contents.append("R. 恢复默认 SMBIOS 型号（{}）".format(default_smbios_model))
            contents.append("")
            ...
            self.utils.head("自定义 SMBIOS 型号", resize=False)
            print(content)
